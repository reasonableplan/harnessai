...
**Phase 15 — v0.15.x (완료, 2026-07-02)**: **원커맨드 드라이버 + dogfood 경화 (원맨툴 P1)**. `/ha-run` — 파이프라인 전체를 상태기계 기준으로 자동 운전하는 단일 명령(`pipeline_advisor.py`, 순수 `advise(plan) → Advice`, 게이트 복제 0; HITL 은 인터뷰 / smoke-FAIL 판단 / ship 확인 3지점만). 가드레일: BLOCK 게이트 자동 우회 금지, 무전이 3회 반복 정지, 세션당 최대 15루프. 실제 URL 단축 CLI 를 `shipped` 까지 완주하며 근본 결함 4건 적발·수정: frozen-게이트 vacuous-pass(HITL-lockable 섹션이 없는 CLI/라이브러리는 `designed→design` 영구 루프 → `requires_hitl_freeze` 헬퍼 + 단일 `HITL_LOCKABLE_SECTIONS` 상수), 프로파일 인지 `print()` 룰(python-cli/claude-skill 정당 stdout 은 WARN 제외, fastapi/lib 는 유지), `ha-smoke` 기동 명령 제안(`smoke:null` CLI 용 `suggest_smoke_command`), file_structure drift 오탐(표준 `src/` 레이아웃 + `docs/` 상태 디렉토리), 프로젝트 유형별 드라이버 reason 정확도. 신규 테스트 +31 (1265 → 1296).

**Phase 16 — v0.16.0 (완료, 2026-07-02)**: **인터뷰 지능화 — capability 추론 & 활성화 정확도 (#1·#3·#11)**. 웹앱 dogfood(nextjs)로 확정한 공통 뿌리 — "6축+프로파일 활성화가 비전문가 의도를 못 잡음". `capability_inference.py` — 설명 텍스트 → `has.*` 신호 결정론 추론(한/영 키워드): "할 일 CRUD" → `storage`, "로그인/계정" → `users`, "API/엔드포인트" → `http_server`, "화면/대시보드" → `ui`; ha-init 이 미활성 신호를 `capability_suggestions` 로 출력 → HITL 확인 → `--external-capabilities` 재작성(자동 활성화 아님 = 결정권 분리). 수정: #3 environments 과활성화를 `required_when` 으로 게이트(poc+casual 장난감 CLI 가 dev/staging/prod · CORS · HSTS 미상속), #1 detect 빈결과 안내. 신규 테스트 +12 (1296 → 1308).

**Phase 17 — v0.17.0 (완료, 2026-07-02)**: **의미 기반 인터뷰 — decision_points 커버리지**. 인터뷰 질문 생성이 전부 어휘(lexical) 정규식(미정량어/실패경로 단어 스캔)이라 비전문가가 남기는 *의미적 빈칸*(다중 사용자·soft delete·동시성·로딩 상태)을 못 잡았다. `decision_coverage.py` 가 fragment frontmatter 신필드 `decision_points`(과거 LESSON/실패 taxonomy 에서 뽑은 필수 결정 항목)를 읽어, 채워진 skeleton 에 그 섹션의 `detect` 키워드가 하나도 없으면 미결정으로 판정(결정론 커버리지 = "언제 멈출지" 정지 조건, 질문은 LLM 담당). 5섹션 11항목 시딩; ha-design `clarify` 에 `decision_candidates`(어휘보다 우선, 강제 질문). v0.16.0 environments 를 validate-red 로 만들던 `required_when` 괄호 false-reject 수정 + 미러 전용 `django` 프로파일 백포트. 신규 테스트 +25 (1308 → 1333).
...
**Phase 20 — v0.20.0 (완료, 2026-07-12)**: **스캐폴딩 흡수 — 결정론 T-000 부트스트랩 + 스텁 스탬퍼**. 빌드가 빈 디렉토리에서 시작하는 바람에 첫 태스크가 `create-next-app` 이 공짜로 주는 것을 손으로 다시 쓰고 있었다. 프로파일에 `toolchain.scaffold` 가 선언돼 있으면 `ha-plan commit` 이 `T-000` 스캐폴드 태스크를 자동 주입하고, `ha-build scaffold` 가 공식 생성기를 mkdtemp 샌드박스에서 돌린 뒤 **기존 파일을 덮지 않고** 병합한다(멱등 — 두 번째 실행은 skip). 스텁 스탬퍼는 태스크가 선언한 파일을 `HARNESS-STUB` 마커와 함께 선생성하고, 마커가 하나라도 남아 있으면 `complete --status done` 이 막힌다 — 쓰지도 않은 파일을 두고 "완료" 라고 말할 수 없다. 신규 테스트 +82 (1408 → 1490).

**Phase 21 — v0.21.x (완료, 2026-07-13)**: **수용검증 계층 + 그것을 증명한 dogfood**. v0.21.0 은 검증 사다리의 최상단 `/ha-accept` 출시 — `/ha-design` 에서 사용자와 합의한 GWT 수용 기준을 (LLM 이) `docs/acceptance.yaml` 로 파생하고, (run.py 가) 기동한 dev 서버에 **결정론적으로** 실행한다. test/lint/type 은 조각이 맞는지, smoke 는 뜨는지, accept 만이 **요구한 대로 동작하는지**를 본다. 파생 가드레일: 시나리오 1개 = GWT 1개, 선언된 엔드포인트만, 파생 불가면 반드시 `underivable` 에 명시 (조용히 빠뜨리는 것이 가장 위험한 커버리지 구멍).

이어 실제 프로젝트(**subtrack** — Next.js 16 + Drizzle/SQLite, T-000 → 8태스크 → verify → smoke → accept → review → `reviewed` 전 구간)로 드라이런해 결함 10건을 발굴하고 v0.21.1 에서 수정했다. 핵심은 **무음으로 비어버린 게이트** — Git Bash 가 `--endpoint /api/x` 를 Windows 경로로 바꾸자 path 파라미터 skip 휴리스틱이 그것을 삼켰고, ha-smoke 는 **아무것도 때리지 않은 채** PASS 를 보고했다(경로가 아닌 endpoint 는 이제 하드 FAIL, probe 는 실제 타격 개수를 항상 보고). 그리고 **끊어진 rework 루프** — `ha-review` REJECT 가 파이프라인을 `building` 으로 되돌리면서 태스크는 `done` 그대로 두어 `/ha-build --resume` 이 dead-end 였다(REJECT 도 ha-verify 처럼 위반 태스크를 `needs_rebuild` 로 전이. 완료 태스크 재진입 시 상태를 되돌리지 않으면 rework 배치의 첫 complete 가 곧바로 `built` 로 전이해 형제 태스크가 게이트를 통과할 기회를 잃는 문제도 함께). 여기에 첫 실제 `create-next-app` 실행이 드러낸 스캐폴드 결함들(샌드박스 임시명이 `package.json` 에 잔존, pnpm `allowBuilds` 플레이스홀더로 비대화형 install 이 항상 실패, pnpm 이 에러를 stdout 에 써서 실패 원인이 유실).

v0.21.2 는 수용검증의 천장을 걷어냈다 — 집계 기준("월 합계 27,000원 표시")은 공유 DB 에서 절대값으로 단언할 수 없는데, **선언형 HTTP 러너 중 캡처값 산술을 지원하는 도구가 없다**(Hurl 은 "셸에서 계산해 `--variable` 로 주입" 을 문서화, Karate/Postman 은 JS 탈출구 — LLM 이 시나리오를 쓰는 구조에서 스크립트 탈출구는 검증 위조 경로다). 그래서 RSpec `change{}.by(delta)` 를 전용 단언으로 이식: `json_delta`(baseline 캡처 후 변화량 단언), `json_not_contains`(부재 — Hurl `not contains` 선례), `{today±N}` 날짜식. subtrack 재파생 결과 시나리오 5 → 8, underivable 4 → 2(둘 다 순수 브라우저 UI), 미커버 feature 0 — 8개 전부 이전 실행이 남긴 데이터로 오염된 DB 위에서 PASS 했고, 이것이 delta 단언의 자기완결성을 증명한다. 신규 테스트 +193 (1408 → 1601).
...
- **테스트**: pytest **1346개** backend + **12개** install 스냅샷 (회귀 0건)
...
backend/
  ...
  tests/                      1346 pytest + skills/ 회귀 방지
...
cd backend
uv sync
uv run pytest tests/ --rootdir=.      # 1346 tests
