...
**Phase 19 — v0.19.x (completed, 2026-07-03)**: **Failure auto-recovery loop + Personal Jira dogfood fixes**. v0.19.0 closes the `/ha-run` driver's defect where "rework regression is normal flow" was designed but not implemented — verify FAIL left rework targets stuck at `done`, so rebuilds were never selected. `plan_manager.mark_for_rebuild()` (done → `needs_rebuild` with reason), ha-verify `record` auto-marks on FAIL + `--rework-tasks`, ha-build `select_ready_tasks` prioritizes `needs_rebuild`, pipeline_advisor surfaces the rework reason; E2E loop test included. v0.19.1 then stress-tested v0.16–0.19 by rebuilding Personal Jira from scratch as a non-expert: all 3 findings fixed — python-cli "tool" false-positive removed from profile recommendation, storage/users keyword expansion in capability inference ("todo management app" no longer loses the persistence section), and decision_coverage no longer false-resolves decision points from unfilled `<...>` placeholders or the fragment's own template scaffolding (headings/checklists/guidance) — a concurrency/TOCTOU question that had been silently suppressed is asked again (dogfood skeleton: 3 suppressed → 0, 11/11 asked). +26 tests (1346 → 1372).
