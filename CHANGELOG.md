...
- `_substitute`: 문자열 전체가 `{var}` 면 타입 보존 (id `7` 이 `"7"` 이 되어 `7 != "7"` 로
  단언이 조용히 어긋나던 문제).
- **전체 감사 후속** — ha-smoke: exit 모드(`--url` 없음)에 `--endpoint` 를 주면 조용히 무시하고
  exit 0 이 나던 것 → 하드 FAIL (D-6 과 동종의 무음 vacuous). ha-review: 재작업 T-ID 파서가
  `T-\d{3}` 3자리를 가정해 `T-1000` 을 놓치던 것 → 캐노니컬 `T-\d+` (task_id.py) 로 정렬.
- 문서 동기: GATES 재집계 (BLOCK 25 → **28**), CLAUDE.md 게이트/테스트 수, README·README.ko
  Phase 20·21 항목 신설 (v0.20.0/v0.21.x 가 통째로 누락돼 있었음), 수용검증 설계서에 v0.21.2
  스키마 확장 절 추가.
