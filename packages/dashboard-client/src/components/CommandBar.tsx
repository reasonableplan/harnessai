...
export default function CommandBar({ onSend }: CommandBarProps) {
  ...
  return (
    <div className="relative px-4 py-2 bg-[#16213e] border-t-2 border-[#0f3460]">
      {/* Suggestions dropdown */}
      {suggestions.length > 0 && (
        <div className="absolute bottom-full left-4 mb-1 bg-[#1a1a3e] border-2 border-[#0f3460] min-w-[180px] z-50">
          {suggestions.map((s, i) => (
            <div
              key={s}
              className={`px-3 py-1.5 font-pixel text-[8px] cursor-pointer ${
                i === selectedSuggestion
                  ? 'bg-[#0f3460] text-cyan-300'
      ...
      <div className="flex items-center gap-2">
        <span className="font-pixel text-[8px] text-cyan-400 select-none">&gt;</span>
        <input
          ref={inputRef}
          type="text"
          value={input}
