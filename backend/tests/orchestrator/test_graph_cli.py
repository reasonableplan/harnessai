...
def test_cli_graph_stdout(tmp_path: Path) -> None:
    ...
    assert "flowchart TD" in result.stdout, f"'flowchart TD' 없음. stdout:\n{result.stdout}"
    # 파일은 변경되지 않아야 함
...
def test_cli_graph_inject_creates_section(tmp_path: Path) -> None:
    ...
    assert _SECTION_HEADER in updated, f"섹션 헤더 없음. 파일 내용:\n{updated}"
...
def test_cli_graph_inject_idempotent(tmp_path: Path) -> None:
    ...
    assert count == 1, f"섹션 헤더가 {count}개 발견 — 중복 추가됨:\n{updated}"
    # mermaid 블록도 1개만
...
def test_cli_graph_inject_creates_backup(tmp_path: Path) -> None:
    ...
    assert backup_files, f"백업 파일 없음. {tmp_path} 내용: {list(tmp_path.iterdir())}"
    # 백업 내용이 원본과 동일해야 함
