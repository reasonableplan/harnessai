...
# Component-reference-bearing sections. §13 holds the canonical component tree;
# §14 (state machine) and §15 (pseudo-code) are where each component must be
# wired in. Other section numbers vary by template, so we cap the search here.
_COMPONENT_DEFINITION_SECTIONS = ("13",)
_COMPONENT_REFERENCE_SECTIONS = ("14", "15")
...
def _split_sections(skel_text: str) -> dict[str, str]:
    """Return {"1": "<body>", "13": "<body>", ...} keyed by heading number."""
    ...
    for i, m in enumerate(matches):
        ...
        sections[m.group(1)] = skel_text[body_start:body_end]
    return sections
...
def check_isolated_components(skel_text: str) -> list[ConsistencyFinding]:
    """Components defined in §13 that never appear in §14 or §15.
    ...
    defined: set[str] = set()
    for sid in _COMPONENT_DEFINITION_SECTIONS:
        if sid in sections:
            defined |= _components_in_section(sections[sid])

    referenced: set[str] = set()
    for sid in _COMPONENT_REFERENCE_SECTIONS:
        if sid in sections:
    ...
    for name in sorted(defined - referenced):
        findings.append(
            ConsistencyFinding(
                ...
                target=name,
                message=(
                    f"Component '{name}' defined in §{_COMPONENT_DEFINITION_SECTIONS[0]} "
                    f"but not referenced in §{'/§'.join(_COMPONENT_REFERENCE_SECTIONS)} — "
...
def check_task_skeleton_references(
    ...
    """Tasks with no §N reference and no §13-component reference in their description.
            ...
            findings.append(
                ConsistencyFinding(
                    ...
                    message=(
                        ...
                        "known §13 component name — implementer may lack anchor."
                    ),
                )
            )
    return findings
