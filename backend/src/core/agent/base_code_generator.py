...
class BaseCodeGeneratorAgent(BaseAgent):
    ...
    def _build_workspace_instructions(self, task: Task, work_dir: str) -> str:
        ...
        return (
            ...
            f"### Step 2: 기존 코드 읽기 + 호환성 확인\n"
            f"작업과 관련된 기존 파일을 읽어서 패턴을 파악하세요:\n"
            f"- backend/app/main.py — 현재 등록된 라우터\n"
            f"- backend/app/models/__init__.py — 사용 가능한 모델\n"
            f"- backend/app/database.py — DB 세션 패턴 (동기/비동기 드라이버 확인)\n"
            f"- backend/requirements.txt — 현재 설치된 패키지 목록\n"
            f"- frontend/package.json — 현재 설치된 npm 패키지 목록\n"
            f"- docker-compose.yml — 서비스 설정, 환경변수\n"
            f"- 기존 라우터/서비스/스키마가 있으면 패턴 참고\n\n"
            f"**호환성 체크 (반드시):**\n"
            f"- 새 import를 추가하면 → requirements.txt 또는 package.json에 해당 패키지가 있는지 확인. 없으면 추가\n"
            f"- DB URL을 사용하면 → database.py의 드라이버(asyncpg/psycopg2)와 일치하는지 확인\n"
            f"- 환경변수를 사용하면 → .env.example과 docker-compose.yml에도 있는지 확인\n"
            f"- 설정 파일(alembic.ini, tsconfig 등)을 수정하면 → 다른 설정 파일과 모순 없는지 확인\n\n"
            f"### Step 3: 코드 작성\n"
            ...
            f"- 새 Python 패키지 import 시 requirements.txt에 추가\n"
            f"- 새 npm 패키지 import 시 package.json에 추가 (npm install --save)\n\n"
            f"### Step 4: 자체 검증\n"
            ...
            f"### Step 4.5: 통합 검증 (다른 파일과의 호환성)\n"
            f"자기 파일만 통과하면 안 됨. 전체 시스템과 호환되는지 확인:\n"
            f"- docker-compose.yml 수정했으면: docker compose config (문법 검증)\n"
            f"- requirements.txt 수정했으면: pip install -r requirements.txt --dry-run\n"
            f"- package.json 수정했으면: npm ls (의존성 트리 검증)\n"
            f"- alembic 관련이면: alembic check 또는 alembic heads\n"
            f"- 프론트엔드이면: npm run build (빌드 성공 확인)\n\n"
            f"### Step 5: 실패하면 직접 수정\n"
            f"위 검증에서 에러가 나면 스스로 원인을 찾아 수정하세요.\n"
            f"모든 검증을 통과할 때까지 반복하세요.\n\n"
            f"### Step 6: 이전 태스크 산출물 참조\n"
            f"이미 완료된 다른 태스크가 만든 파일이 workspace에 있습니다.\n"
            f"코드를 작성하기 전에 ls나 find로 현재 파일 목록을 확인하세요.\n"
            f"특히 다음을 확인:\n"
            f"- backend/app/models/ — 이미 정의된 모델이 있으면 재정의하지 말고 import\n"
            f"- backend/app/routers/ — 이미 등록된 라우터가 있으면 패턴 참고\n"
            f"- backend/requirements.txt — 이미 추가된 패키지 확인\n"
            f"- frontend/src/ — 이미 만들어진 컴포넌트/타입 확인\n"
            f"- frontend/package.json — 이미 추가된 의존성 확인\n"
            f"**절대로 이미 존재하는 파일을 중복 생성하거나 내용을 덮어쓰지 마세요.**\n\n"
            f"## 금지사항\n"
