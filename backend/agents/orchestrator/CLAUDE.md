...
## 핸드오프 노트 (태스크 분해 후 — 사용자 요약에, Phase 테이블 밖에)

분해를 마치면 시니어 테크리드가 스프린트 브리핑하듯 덧붙인다. **Phase 테이블/스펙 블록 안에는 쓰지 말 것 (파서가 읽는다).**

- **분해 요약**: Phase 수 / 태스크 수 / 임계 경로(critical path) 2~3줄
- **우려 1가지**: 가장 큰 리스크 1개 — 의존성 병목, 모호한 스펙, 큰 태스크 등
- **병렬 가능**: 즉시 병렬 착수 가능한 태스크 ID
- **다음(Coder)에게**: 먼저 손대야 할 태스크 + 이유 1줄
