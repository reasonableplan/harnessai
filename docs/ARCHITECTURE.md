...
`backend/docs/shared-lessons.md` 에 과거 실수 20개 (LESSON-001 ~ LESSON-020). 각 프로파일의 `lessons_applied` 필드가 적용 대상 지정.

적용 메커니즘:
- **텍스트 참조** (default): Reviewer 에이전트가 프롬프트에 포함해 판단
- **자동 감지** (LESSON-018 만 현재): ai-slop 정규식 패턴에 통합

LESSON 추가는 `/my-lessons` (v1) 또는 수동. 자동 학습 (`/ha-review` 반복 패턴 → LESSON 후보) 은 TODOS.md 에 등록된 향후 작업.
...
| D3 | `/my-*` 완전 삭제 → `/ha-*` (single cut-over, Phase 4) | 스킬 중복 유지 비용 |
